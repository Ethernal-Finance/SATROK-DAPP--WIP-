...
contract  SatoshiRoku is ERC20, ReentrancyGuard {
    string public constant symbol = "sqne"; // Squid Network
    string public constant name = "Squid Network"; 
 ...
 //Tax reserves and pots kept public for maximum transparency  
   ...
   uint32 public SellTaxPercentage = 10;
...
function setStakingOwnership(address newStakeOwner) public onlyMain(){// For setting new stake handling contract
  ...
  //  address constant routerAddress=0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    //MainNet
    address constant routerAddress=0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; // UNI-SWAP ROUTER v2
    ...
    constructor() {
        ...
        nonce = 1;
        burnUntil = initialSupply/8; //burns up to  87.5% initial supply 1b/8=125000000
...
//        __balanceOf[address(this)] = __totalSupply/2;
   ...
   AddTokentoPayoutList(_pancakeRouter.WETH(), "ETH"); //ETH added as default
   AddTokentoPayoutList(address(this), "SQNE"); //this contract token
   AddTokentoPayoutList(address(0x8aAc9dEe1D46F3Bd870489b596067DC8E0741bf6), "BTC"); //BTC
   AddTokentoPayoutList(address(0x8aAc9dEe1D46F3Bd870489b596067DC8E0741bf6), "Toncoin");
   AddTokentoPayoutList(address(0x8aAc9dEe1D46F3Bd870489b596067DC8E0741bf6), "Imperial obelisk");
   AddTokentoPayoutList(address(0x8aAc9dEe1D46F3Bd870489b596067DC8E0741bf6), "Hex");
   AddTokentoPayoutList(address(0x8aAc9dEe1D46F3Bd870489b596067DC8E0741bf6), "Volt inu");
   AddTokentoPayoutList(address(0x8aAc9dEe1D46F3Bd870489b596067DC8E0741bf6), "Saitama");
   AddTokentoPayoutList(address(0x8aAc9dEe1D46F3Bd870489b596067DC8E0741bf6), "Shiryo inu");
   AddTokentoPayoutList(address(0x8aAc9dEe1D46F3Bd870489b596067DC8E0741bf6), "Usdc");
   AddTokentoPayoutList(address(0x8aAc9dEe1D46F3Bd870489b596067DC8E0741bf6), "Usdt");
   AddTokentoPayoutList(address(0x8aAc9dEe1D46F3Bd870489b596067DC8E0741bf6), "Shiba");
   AddTokentoPayoutList(address(0x8aAc9dEe1D46F3Bd870489b596067DC8E0741bf6), "BTC");
   }
...
{   
    if(SellTaxBNBPot>10000)  //skip if pot is empty
    {
         ...
         //50% holder BNB @@@
            HolderPotBNB += SellTaxBNBPot * 30 / 100;  //adds to holder reward pot
     
       //19% staking BNB   @@@
            MarketingBNB += SellTaxBNBPot * 19 / 100;
...
function DistributeBuyTax() internal
{
     if(BuyTaxBNBPot>100)
     {
        ...
        //10% ges fees => 14%
        GasPotBNB += BuyTaxBNBPot * 14 / 100; //14% goes to gas fees
        
        //35% holders in bnb => 40%
        HolderPotBNB += BuyTaxBNBPot * 30 / 100;
        
        //19% Staking bnb =>10%
        
         MarketingBNB += BuyTaxBNBPot * 30 / 100;

        //4% admin in bnb => 6%
        
        AdminPotBNB += BuyTaxBNBPot * 16 / 100;
...
{
    require(newBuyTax+newSellTax <= 20,"total tax cannot exceed 20%"); //For holder safety
...
function ClaimRewards(address receiver) private returns(bool){
  ...
  {  _LastPayout[receiver] = block.timestamp;
    ...
    slices[5] =   _RewardTokens[receiver].Slice6;
      ...
      if(PayoutTokenContract == _pancakeRouter.WETH()) //payout ETH
    {
...
function BNB2BUSDconverter(uint256 BNBs) public view returns (uint256)
{
    address[] memory path = new address[](2);
        path[0] = _pancakeRouter.WETH();
        path[1] = address(0x8aAc9dEe1D46F3Bd870489b596067DC8E0741bf6); //mainnet USDT
